...
import { FolderKanban, FileText, GraduationCap, type LucideIcon } from 'lucide-react'
...
export default function DashboardHomePage() {
  const [counts, setCounts] = useState({ categorias: 0, posts: 0, cursos: 0 })
  ...
  useEffect(() => {
    Promise.all([
      ...
      getPosts({ limit: 1 }),
      getCourses(1, 100),
    ]).then(([categorias, posts, cursos]) => {
      setCounts({
        categorias: categorias.meta.totalItems,
        posts: posts.meta.totalItems,
        cursos: cursos.items.length, // aproximado: /cursos no expone un total real
      })
    })
  }, [])
